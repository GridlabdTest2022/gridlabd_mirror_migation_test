...
n_tested = 0
n_passed = 0
exename = '/opt/gridlabd/bin/gridlabd'
dry_run = False
show_debug = False
show_failure = False
show_verbose = False
...
def runtest(workdir,glmname) :
	...
	rc = run_command("/opt/gridlabd/bin/python3 %s/source/link/python/python_gridlabd.py -W %s %s 1>gridlabd.out 2>&1" % (owd,workdir,glmname))
	if not dry_run:
		if rc == 255 :
